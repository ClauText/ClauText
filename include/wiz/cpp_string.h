...
namespace wiz{
	...
	class StringTokenizer2 // with empty token?
	{
	private:
		std::vector<std::string> _m_str;
		int count;
	...
	public:
		...
		explicit StringTokenizer2(const std::string& str, const std::string& separator, StringBuilder* builder, int option = 0)
			: count(0), exist(false), option(option)
		{
			std::vector<std::string> vec; vec.push_back(separator);
			Init(str, vec, builder);
		}
		explicit StringTokenizer2(std::string&& str, const std::string& separator, StringBuilder* builder, int option = 0)
		{
			std::vector<std::string> vec; vec.push_back(separator);
			Init(std::move(str), vec, builder);
		}
		explicit StringTokenizer2(const std::string& str, const std::vector<std::string>& separator, StringBuilder* builder, int option = 0)
			: count(0), exist(false), option(option)
		{
			Init(str, separator, builder);
		}
		explicit StringTokenizer2(std::string&& str, const std::vector<std::string>& separator, StringBuilder* builder, int option = 0)
			: count(0), exist(false), option(option)
		{
			Init(std::move(str), separator, builder);
		}

		explicit StringTokenizer2(const std::string& str, StringBuilder* builder, int option = 0)
			: count(0), exist(false), option(option)
		{
			Init(str, whitespaces, builder);
		}
		explicit StringTokenizer2(std::string&& str, StringBuilder* builder, int option = 0)
			: count(0), exist(false), option(option)
		{
			Init(std::move(str), whitespaces, builder);
		}
