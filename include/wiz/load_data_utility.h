				...
				void operator() () {
					{
						...
						for (char* x = start; x < last; ++x) {
							...
							else if (0 == state && (*x) == option->Assignment[0]) {//-1 != (idx = checkDelimiter(x, last, option->Assignment))) {
								token_last = x - 1;
							...
							else if (0 == state && (*x) == option->LineComment) { // different from load_data_from_file
								token_last = x - 1;
								if (token_last >= 0 && token_last - token_first + 1 > 0) {
									aq->emplace_back(token_first, token_last - token_first + 1, false);
									token.clear();
